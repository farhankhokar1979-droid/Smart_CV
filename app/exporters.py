...
def export_pdf(resume):
    ...
    # Lazy import so the app doesn't crash on startup if WeasyPrint
    # isn't available on the hosting platform.
    from weasyprint import HTML

    body_html = render_resume(resume)

    # Bundled, offline-safe fonts
    fonts_dir = os.path.join(os.path.dirname(__file__), "static", "fonts")
    inter_path = "file://" + os.path.join(fonts_dir, "Inter-Regular.ttf").replace("\\", "/")
    jakarta_path = "file://" + os.path.join(fonts_dir, "PlusJakartaSans.ttf").replace("\\", "/")
...
def export_docx(resume):
    """Build a clean, ATS-friendly DOCX directly from section data."""
    ...
    accent_hex = style["primary_color"].lstrip("#")
    accent_rgb = (
        RGBColor(*(int(accent_hex[i:i + 2], 16) for i in (0, 2, 4)))
        if len(accent_hex) == 6
        else RGBColor(0x6D, 0x28, 0xD9)
    )
